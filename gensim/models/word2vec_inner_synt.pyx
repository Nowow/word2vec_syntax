...
def train_batch_sg(model, sentences, alpha, _work, synt_cash):
    ...
    cdef np.uint32_t indexes_cont[MAX_SENTENCE_LEN]
    cdef np.uint32_t reduced_windows[MAX_SENTENCE_LEN]
    cdef int word_related_cont[MAX_SENTENCE_LEN]
    cdef int sentence_idx[MAX_SENTENCE_LEN + 1]
    cdef int window = model.window

    cdef int i, j, k
    cdef int effective_words = 0, effective_sentences = 0
    cdef int cont_length = 0
    ...
    for cont in synt_cash:
        if not cont:
            continue  # ignore empty sentences; leave effective_sentences unchanged
        
        word = vlookup[cont[0]] if cont[0] in vlookup else None
        if word is None:
        ...
        word_c = 0   
        for objec in cont[1:]:
            objec = vlookup[objec] if objec in vlookup else None
            if objec is None:
                continue
            if sample and objec.sample_int < random_int32(&next_random):
                continue
            indexes_cont[word_c] = objec.index
            word_c += 1
            cont_length += 1
            
        
        word_related_cont[effective_words] = cont_length
        cont_length = 0
    ...
    cdef int cont_words = 0

    
    # release GIL & train on all sentences
    with nogil:
    # PEREDELAT VSE ZDES. TUT NADO VSE POMENJAT В
        for wd in range(effective_words):
            for i in range(word_related_cont[wd]):
                if hs:
                    fast_sentence_sg_hs(points[wd], codes[wd], codelens[wd], syn0, syn1, size, indexes_cont[cont_words], _alpha, work, word_locks)
                cont_words += 1
                
            
        
        
#        for context_pair in synt_cash:
#            if not context_pair:
#                continue
#
#            if hs:
#                word = vlookup[context_pair[0]] if context_pair[0] in vlookup else None
#                if word is None:
#                    continue
#                if sample and word.sample_int < random_int32(&next_random):
#                    continue
#                
#                for objec in context_pair[1:]:
#                    objec = vlookup[objec] if objec in vlookup else None
#                    if objec is None:
#                        continue
#                    if sample and objec.sample_int < random_int32(&next_random):
#                        continue
#                    fast_sentence_sg_hs(<np.uint32_t *>np.PyArray_DATA(word.point), <np.uint8_t *>np.PyArray_DATA(word.code), <int>len(word.code), syn0, syn1, size, objec.index, _alpha, work, word_locks)
            
#            else:
#                print('ALERT ALERT NOT HS NOT ABLE TO COMPREHEND ANYTHING ABORTING')
